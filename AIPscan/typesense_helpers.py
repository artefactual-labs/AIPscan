...
def augment_document_with_aip_data(table, document, aip_cache, result):
    if table == "aip":
        document["original_file_count"] = aip_cache["original_file_counts"][result.id]

    if table == "file":
        document["storage_service_id"] = aip_cache["storage_service_ids"][result.aip_id]
        document["storage_location_id"] = aip_cache["storage_location_ids"][
            result.aip_id
        ]

        # Round AIP create date to the day
        dt = aip_cache["create_dates"][result.aip_id]
        dt = datetime.datetime(*dt.timetuple()[0:3])
        document["aip_create_date"] = int(dt.strftime("%s"))


def populate_index(update_threshold_percent=1):
    ts_client = client()

    # Cache AIP storage services/locations
    aip_cache = {}
    aip_cache["storage_service_ids"] = {}
    aip_cache["storage_location_ids"] = {}
    aip_cache["create_dates"] = {}
    aip_cache["original_file_counts"] = {}

    results = AIP.query.all()

    for result in results:
        aip_cache["storage_service_ids"][result.id] = result.storage_service_id
        aip_cache["storage_location_ids"][result.id] = result.storage_location_id
        aip_cache["create_dates"][result.id] = result.create_date
        aip_cache["original_file_counts"][result.id] = result.original_file_count
    ...
    for model in APP_MODELS:
        ...
        for result in results:
            ...
            augment_document_with_aip_data(table, document, aip_cache, result)
