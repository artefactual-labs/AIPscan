...
def test_create_numbered_subdirs(timestamp, package_list_number, mocker):
    """Ensure that the logic that we use to create sub-directories for
    storing METS is sound.
    """

    download_root = "/tmp/aipscan-downloads"
    expected_path = os.path.join(
        download_root, timestamp, "mets", str(package_list_number)
    )

    mocker.patch(
        "AIPscan.Aggregator.task_helpers.get_download_root", return_value=download_root
    )
