...
def test_augment_document_with_aip_data(app_instance):
    document = {}

    aip_cache = {
        "original_file_counts": {3: 23},
        "storage_service_ids": {3: 1},
        "storage_location_ids": {3: [2]},
        "create_dates": {3: datetime(2024, 3, 2, 5, 35)},
    }

    class MockAipResult:
        id = 3

    aip_result = MockAipResult()

    ts_helpers.augment_document_with_aip_data("aip", document, aip_cache, aip_result)

    assert document["original_file_count"] == 23
    ...
    ts_helpers.augment_document_with_aip_data("file", document, aip_cache, file_result)
